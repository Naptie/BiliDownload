...
4. *【如果从未保存过 SESSDATA 或 cookie 已失效】* 输入 cookie 中 SESSDATA 的值（若无则填“#”）。
...
5. 如果程序的下载速度慢，请自行访问下载地址进行下载。

6. 对于上述使用流程中的所有输入，均可写入工作目录下的 `Input.txt` 中。

7. 本程序仍然存在诸如不支持 4K 与 1080P+ 清晰度的无水印下载、下载速度慢等问题，欢迎大家多多投递 pull request。
